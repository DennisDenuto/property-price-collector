...
var _ = Describe("BatchTrainingDataRepo", func() {
	...
	BeforeEach(func() {
		...
		trainingDataRepo = NewBatchTrainingDataRepo(NewTrainingDataRepo(fakeApiClient), 1)
	})
	...
	Describe("Commit", func() {
		...
		It("should not finish transaction if batch size is not met", func() {
			Expect(trainingDataRepo.Commit("master-123")).To(Succeed())

			Expect(fakeApiClient.FinishCommitCallCount()).To(Equal(0))
		})
